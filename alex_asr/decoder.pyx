...
cdef extern from "src/decoder.h" namespace "alex_asr":
    cdef cppclass _Decoder "alex_asr::Decoder":
        ...
        void SetSpkrID(string spkr_ID) except +
        string GetSpkrID() except +
        vector[string] GetSpkrList() except +
...
cdef class Decoder:
    ...
    def set_spkrID(self, sid):
        self.thisptr.SetSpkrID(sid)

    def get_spkrID(self):
        return self.thisptr.GetSpkrID()

    def get_spkrList(self):
        # this will return a list of speakers in the transform file
        cdef vector[string] c_spkrList 
        c_spkrList = self.thisptr.GetSpkrList()

        spkrList = [c_spkrList[i] for i in xrange(c_spkrList.size())]
